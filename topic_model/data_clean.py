...
def sentence_tokenize(doc,_type='LEMMA'):#splits sentences of a doc where each sentence is list of words ; does stemming 
	sent_tokenize_list = sent_tokenize(doc)
	sent_list = []
	
	for sentence in sent_tokenize_list:
		sentence = [word for word in nltk.word_tokenize(sentence.lower()) if word not in string.punctuation]
		if _type=='STEM':
...
def tokenize_sentence(sentence,_type='LEMMA'):
	...
	word_tokenize_list = [w.lower() for w in word_punct_tokenizer.tokenize(sentence)]
