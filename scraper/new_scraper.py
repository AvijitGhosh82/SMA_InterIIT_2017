...
client = ArangoClient(host="127.0.0.1", port=8529,
...
class Worker(Thread):
    ...
    def __init__(self, queue, discovery, socks_proxy_port):
        ...
        self.opener = urllib2.build_opener(SocksiPyHandler(
            socks.PROXY_TYPE_SOCKS4, 'localhost', self.socks_proxy_port))
        self.conn = ArangoClient(host="127.0.0.1", port=8529,
                                 username='root', password='')
        self.db = client.db('Scraping')
        self.coll = client.db('Scraping').collection('imdb_ratings')
    ...
    def run(self):
        while True:
            try:
                ...
                self.coll.update({'index': index}, {
                                 '$set': {'last_response': code}})
...
class Croupier(Thread):
    ...
    def run(self):
        # Look if imdb collection is empty. Only if its empty we create all the
        # items
        c = self.coll.count()
        if c == 0:
            print "Inserting items"
            # self.coll.ensure_index(
            # [('index', pymongo.ASCENDING), ('last_response', pymongo.ASCENDING)])
            for i in xrange(Croupier.Base, Croupier.Top):
                self.coll.insert(
                    {'index': i, 'url': url_format.format(i), 'last_response': 0})
        ...
        while True:
            ...
            items = self.coll.find({'$and': [{'last_response': {'$ne': 200}}, {
                                   'last_response': {'$ne': 404}}]})
